...
rule("Genji 技能介绍  显示")
{
	...
	actions
	{
		...
		Create HUD Text(Event Player, Null, Null, Custom String("特殊技能2: 无相  {0} + {1}  ", Custom String("换弹"), Input Binding String(Button(
			Primary Fire))), Top, 8, Color(White), Color(White), Color(Lime Green), Visible To and String, Default Visibility);
		Create HUD Text(Event Player, Null, Null, Custom String("特殊技能 3: 雾隐 = {0} + {1} (お前はもう死んでいる) ", Custom String("跳跃"), Custom String(
			"换弹")), Top, 9, Color(White), Color(White), Color(Lime Green), Visible To and String, Default Visibility);
...
rule("Genji 技能冷却  显示")
{
	...
	actions
	{
		Create HUD Text(Event Player, Null, Null, Custom String("{0} {1} {2}", Custom String("特殊技能 1: 突"), Custom String("  "),
			Event Player.abilCooldown2), Right, 1, Color(Green), Color(Green), Color(Lime Green), Visible To and String,
			Default Visibility);
		Create HUD Text(Event Player, Null, Null, Custom String("{0} {1} {2}", Custom String("特殊技能 2: 御"), Custom String("  "),
			Event Player.abilCooldown1), Right, 2, Color(White), Color(White), Color(Lime Green), Visible To and String,
			Default Visibility);
		Create HUD Text(Event Player, Null, Null, Custom String("{0} {1} {2}", Custom String("特殊技能 3: お前はもう死んでいる"), Custom String("  "),
...
rule("简易前突进")
{
	...
	actions
	{
		disabled Apply Impulse(Event Player, Vector(0, 0, 1), 50, To Player, Cancel Contrary Motion);
...
rule("Genji 弹反地上-执行")
{
	...
	actions
	{
		...
		Set Status(Event Player.enemy, Null, Stunned, 1.350);
		Event Player.GroundStrike = Player Closest To Reticle(Event Player, Opposite Team Of(Team Of(Event Player)));
...
disabled rule("测试 反弹技能1")
{
	...
	actions
	{
		...
		disabled Damage(Event Player, Null, 100);
		Apply Impulse(Event Player, Vector(X Component Of(Facing Direction Of(Event Player)) * -1, 0.100, Z Component Of(
			Facing Direction Of(Event Player)) * -1), 10, To World, Cancel Contrary Motion);
		Apply Impulse(Players In Slot(0, Opposite Team Of(Team Of(Event Player))), Vector(X Component Of(Facing Direction Of(
			Players In Slot(-1, Opposite Team Of(Team Of(Event Player))))) * -1, 0.100, Z Component Of(Facing Direction Of(Players In Slot(
